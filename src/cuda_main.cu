...
__global__ void brute_force_collisions_and_gravity(
    float* pos_x, float* pos_y,
    float* vel_x, float* vel_y,
    uint* mass, uint* radius, 
...
extern "C" void collisions_and_gravity(uint no_of_particles, float gravitational_const) {

    int threads = 1024;
    int blocks = (no_of_particles + threads - 1)/ threads;

    brute_force_collisions_and_gravity<<<blocks, threads>>> ( 
    d_pos_x, d_pos_y, 
    d_vel_x, d_vel_y, 
    d_mass, d_radius,  
    no_of_particles, gravitational_const
    );
